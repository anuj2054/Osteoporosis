...
if mysourcelocation=='1'
if isempty(isource), isource = fix(nx/2); end;
if isempty(jsource), jsource = fix(ny/2); end;
end

if mysourcelocation=='2'
if isempty(isource), isource = nx-npoints_pml; end;
if isempty(jsource), jsource = fix(ny/2); end;
end
...
if isempty(xfin), xfin = (isource - radius) .* deltax; end;
...
if isempty(sisp), sisp=zeros(nstep/1000,nrec); end;
...
if (mysourcelocation =='1')
if(((i-fix(nx/2)).^2+(j-fix(ny/2)).^2)>=(radius.^2))
rho(i,j) = density2;
...
sigmaxx(i,j) = sigmaxx(i,j) +(lambda_plus_two_mu_half_x .* value_dvx_dx) .* deltat; % check the validity of this statement
...
vx(i,j) = vx(i,j) +(value_dsigmaxx_dx) .* deltat ./ rho(i,j);
end; i =fix(nx+1);
%end; j =fix(ny+1);
...
sisp(it+1,irec) = sigmaxx(ix_rec(irec),iy_rec(irec));
...
total_energy_kinetic(it+1) = 0.5d0 .* sum(sum(rho([npoints_pml+1:nx-npoints_pml],[npoints_pml+1:ny-npoints_pml]).*(vx([npoints_pml+1:nx-npoints_pml],[npoints_pml+1:ny-npoints_pml]).^2 )));
...
total_energy_potential(it+1) = total_energy_potential(it+1) +0.5d0 .*(epsilon_xx .* sigmaxx(i,j));
end; i = fix(nx-npoints_pml+1);
%end; j = fix(ny-npoints_pml+1);
% output information
if(rem(it-1,it_display) == 0)
% print maximum of norm of velocity
tempmaxval=sqrt(vx.^2 );
...
title(' Velocity vs. Time at x=50mm');
%saveas(gcf,'./Vx1.jpg');
%saveas(gca,'./Vx1.jpg');
...
xlabel('Time (in seconds)');
ylabel({'Velocity',' (in mm/s)'});
title(' Velocity vs. Time at x=25mm');
%saveas(gcf,'./Vx2.jpg');
%saveas(gca,'./Vx2.jpg');
...
title(' Pressure vs. Time at x=50mm' );
%saveas(gcf,'./pf1.jpg');
%saveas(gca,'./pf1.jpg');
...
xlabel('Time (in seconds)');
ylabel({'Pressure','(in Pa)'});
title(' Pressure vs. Time at x=25mm');
%saveas(gcf,'./pf2.jpg');
%saveas(gca,'./pf2.jpg');

%%%%%%%%%%%%%%%%%%55
filename1= sprintf('1D_elastic_loc=%s_type=%s.jpg',mysourcelocation,sourcetype)
filename2= sprintf('1D_elastic_loc=%s_type=%s.fig',mysourcelocation,sourcetype)
...
clear a a_x a_x_half a_y a_y_half abscissa_in_pml abscissa_normalized alpha_max_pml alpha_prime_x alpha_prime_x_half alpha_prime_y alpha_prime_y_half angle_force b_x b_x_half b_y b_y_half courant_number cp cs d0_x d0_y d_x d_x_half d_y d_y_half degrees_to_radians deltat deltax deltay density dist distval epsilon_xx epsilon_xy epsilon_yy f0 factor force_x force_y hugeval i irec isource it it_display ix_rec iy_rec j jsource k_max_pml k_x k_x_half k_y k_y_half lambda lambda_half_x lambda_plus_two_mu_half_x memory_dsigmaxx_dx memory_dsigmaxy_dx memory_dsigmaxy_dy memory_dsigmayy_dy memory_dvx_dx memory_dvx_dy memory_dvy_dx memory_dvy_dy mu mu_half_x mu_half_y npoints_pml npower nrec nstep nx ny pi rcoef rho rho_half_x_half_y sigmaxx sigmaxy sigmayy sisvx sisp sisvy source_term stability_threshold t t0 thickness_pml_x thickness_pml_y total_energy_kinetic total_energy_potential use_pml_xmax use_pml_xmin use_pml_ymax use_pml_ymin value_dsigmaxx_dx value_dsigmaxy_dx value_dsigmaxy_dy value_dsigmayy_dy value_dvx_dx value_dvx_dy value_dvy_dx value_dvy_dy velocnorm vx vy xdeb xfin xoriginleft xoriginright xrec xsource xspacerec xval ydeb yfin yoriginbottom yorigintop yrec ysource yspacerec yval zero ; 
