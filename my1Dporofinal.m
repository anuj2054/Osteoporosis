...
sourcelocation=input('center or side','s')
sourcetype=input(' [1] Gaussian [2] Derivative of Gaussian [3] Second Derivative of Gaussian ','s')
%%
...
if sourcelocation=='2'
if isempty(isource), isource = nx-npoints_pml+1; end;
if isempty(jsource), jsource = fix(ny./2) +1; end;
end
...
if isempty(angle_force), angle_force = 135.0d0; end;
% receivers
...
if isempty(xfin), xfin =deltax .* (fix(nx/2)+1-radius); end;
...
sigma2(i+1,j+1)=-alpha(i+1,j+1).*rbm(i+1,j+1).*(gamma11(i+1,j+1))+rbm(i+1,j+1).*(xi_1(i+1,j+1));
end; i =fix(nx-1+1);
%end; j =fix(ny+1);
...
for i = 1:nx-1;
sigmaxx(i+1,j+1)=(rlambdao(i+1,j+1)+2.0d0.*rmu(i+1,j+1)).*gamma11(i+1,j+1) -alpha(i+1,j+1).*sigma2(i+1,j+1);
...
for irec = 1:nrec;
% solid velocity 
sisvx(it,irec) = vx(ix_rec(irec)+1,iy_rec(irec));
% fluid pressure
sisp(it,irec) = sigma2(ix_rec(irec)+1,iy_rec(irec));
% fluid velocity 
sisvxf(it,irec) = vxf(ix_rec(irec)+1,iy_rec(irec));
% solid stress 
sissigmaxx(it,irec) = sigmaxx(ix_rec(irec)+1,iy_rec(irec));
...
if max(vx(:,jsource+1))>tempmax
    tempmax=max(vx(:,jsource+1))
end

plot(vx(:,jsource+1))
axis([0 nx -1*tempmax tempmax]); 
frame=getframe();
...
ylabel({'Solid Velocity','(in mm/s)'});
title(' Solid Velocity vs. Time AT SOURCE');
%saveas(gcf,'./Vx1.jpg');
%saveas(gca,'./Vx1.jpg');
...
xlabel('Time (in seconds)');
ylabel({'Solid Velocity','(in mm/s)'});
title(' Solid Velocity vs.Time AT EDGE');
%saveas(gcf,'./Vx2.jpg');
%saveas(gca,'./Vx2.jpg');
...
% Solid Stress
stress_file=zeros(nstep,2);
fid_20=fopen('./stress1.dat','w+');
for it=1:nstep;
stress_file(it,1)= ((it-1).*deltat - t0);
stress_file(it,2)= sissigmaxx(it,1);
fprintf(fid_20,['%0.15g %0.15g \n'],stress_file(it,1) , stress_file(it,2));
end; it =fix(nstep+1);
fclose(fid_20);

%plot it
subplot(4,1,3);
plot(stress_file(:,1),stress_file(:,2));
xlabel('Time (in seconds)');
ylabel({'Solid Stress','(in Pa)'});
title('Solid Stress vs. Time AT SOURCE' );
%saveas(gcf,'./stress1.jpg');
%saveas(gca,'./stress1.jpg');

%%%%%%%%%%%%%%%%%%%%

% Solid Stress
stress_file=zeros(nstep,2);
fid_20=fopen('./stress2.dat','w+');
for it=1:nstep;
stress_file(it,1)= ((it-1).*deltat - t0);
stress_file(it,2)= sissigmaxx(it,2);
fprintf(fid_20,['%0.15g %0.15g \n'],stress_file(it,1) , stress_file(it,2));
end; it =fix(nstep+1);
fclose(fid_20);
...
xlabel('Time (in seconds)');
ylabel({'Solid Stress','(in Pa)'});
title('Solid Stress vs. Time AT EDGE' );
%saveas(gcf,'./stress2.jpg');
%saveas(gca,'./stress2.jpg');

%%%%%%%%%%%%%%%%%%55


filename1= sprintf('1D_Poroelastic_solid_loc=%s_type=%s.jpg',sourcelocation,sourcetype)
filename2= sprintf('1D_Poroelastic_solid_loc=%s_type=%s.fig',sourcelocation,sourcetype)
...
xlabel('Time (in seconds)');
ylabel({'Fluid Velocity','(in mm/s)'});
title('Fluid Velocity vs. Time AT SOURCE');
%saveas(gcf,'./Vxf1.jpg');
%saveas(gca,'./Vxf1.jpg');
...
xlabel('Time (in seconds)');
ylabel({'Fluid Velocity','(in mm/s)'});
title('Fluid Velocity vs. Time AT EDGE');
%saveas(gcf,'./Vxf2.jpg');
%saveas(gca,'./Vxf2.jpg');
...
filename1= sprintf('1D_Poroelastic_fluid_loc=%s_type=%s.jpg',sourcelocation,sourcetype)
filename2= sprintf('1D_Poroelastic_fluid_loc=%s_type=%s.fig',sourcelocation,sourcetype)
saveas(gcf,filename1);
saveas(gca,filename1);
saveas(gcf,filename2);
saveas(gca,filename2);
